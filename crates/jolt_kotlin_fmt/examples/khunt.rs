...
// the imported real-world corpora in tools/import/.imports.
// Not part of the test suite; run with `cargo run -p jolt_kotlin_fmt --example hunt --release`.
//
...
fn main() {
    ...
    for suite in ["ktfmt/source", "maplibre-compose/source"] {
        collect(&imports.join(suite), &mut files);
    }
    files.sort();

    let out_dir = root.join("target/hunt/kotlin");
...
fn check_source(path: &Path, source: &str) -> Vec<Failure> {
    ...
    // Inject a block comment at token boundaries and re-check. Small files
    // get a probe at every boundary; large files get pseudo-random probes.
    // Line comments stay out of the sweep: they are a documented known gap
    // in the Kotlin comment-position coverage.
