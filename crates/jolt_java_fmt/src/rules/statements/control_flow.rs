...
use jolt_java_syntax::{
    EnhancedForVariableSyntax, ForInitializer, ForInitializerSyntax, ForStatementFormSyntax,
    ForUpdate, JavaSyntaxField, StatementExpressionList,
};
...
pub(super) fn format_if_statement<'source>(
    ...
    let (then_doc, then_forces_line) = match resolve_required_field(statement.then_branch(), doc) {
...
pub(super) fn format_for_statement<'source>(
    ...
    format_required_field(statement.form(), doc, |form, doc| match form.classify() {
        Ok(ForStatementFormSyntax::BasicForStatement(basic)) => {
            format_basic_for_statement(&basic, doc)
        }
        Ok(ForStatementFormSyntax::EnhancedForStatement(enhanced)) => {
...
fn format_basic_for_statement<'source>(
    ...
    let initializer = match resolve_optional_field(statement.initializer(), doc) {
        JavaFormatField::Present(value) => value.map(|value| format_for_initializer(&value, doc)),
...
fn format_for_initializer<'source>(
    ...
    format_required_field(value.value(), doc, |value, doc| match value.classify() {
        Ok(ForInitializerSyntax::LocalVariableDeclaration(declaration)) => {
            format_local_variable_declaration(&declaration, doc)
        }
        Ok(ForInitializerSyntax::StatementExpressionList(expressions)) => {
