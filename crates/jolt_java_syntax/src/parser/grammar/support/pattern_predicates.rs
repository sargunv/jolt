...
impl Parser<'_> {
    pub(in crate::parser::grammar) fn starts_case_type_pattern(&self) -> bool {
    ...
    pub(in crate::parser::grammar) fn starts_pattern(&self) -> bool {
        self.starts_case_type_pattern() || self.starts_record_pattern()
    }

    pub(in crate::parser::grammar) fn starts_record_pattern(&self) -> bool {
        let mut lookahead = self.lookahead();
        lookahead.skip_variable_modifiers();
        if !lookahead.at_non_void_type_start() || lookahead.starts_literal_expression() {
            return false;
