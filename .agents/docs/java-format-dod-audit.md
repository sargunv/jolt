...
- Current fixture count: 53 style input fixtures.
...
- Formatter ignore ranges are covered for block-statement sequences by
  `statements/formatter-ignore-block`; declaration/member body ignore ranges
  still need equivalent coverage before the general rule is proven.
