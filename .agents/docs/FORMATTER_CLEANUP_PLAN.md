...
Status: complete through draft PR 34\
Started: 2026-07-22\
...
- replace PR 15's Java-local `VisibleDoc` and malformed `(Doc, bool)` tuple with
  PR 16's existing `LayoutDoc` representation;
- delete one-use wrappers and duplicated visible/claim-only constructors while
  leaving Java and Kotlin separator and joining policy language-local;
- preserve output, syntax/document topology, and realistic allocations while
  requiring net production deletion.

### PR 24 — Bounded flat Java annotation lookahead

- extend PR 21's lazy one-word-per-token lookahead summary with exact,
  path-compressed endpoints for maximal annotation runs;
- batch modifier annotation runs through the same summary so top-level, import,
  member, and rejected-constructor recovery cannot rescan each suffix;
- retain one annotation grammar in `skip_annotation`, exact recovery boundaries,
  and one `O(N)` table/query algorithm with measured common-path allocation;
- do not add a second annotation memo, eager grammar copy, or fixed syntax cap.

### PR 25 — Bounded recursive Java values

- reuse PR 22's parser-wide 128-owner policy with a separate active syntax-owner
  counter; generic lookahead and consumption retain their own counter so both
  make the same branch decision;
- guard assignment, unary, annotation-element-value, and nested
  array-initializer cycles, including their cross-family re-entry paths;
- preserve the first unsupported value as one diagnostic-owned bogus expression,
  or initializer using allocation-free owner-local endpoint scans;
- require depth-edge and 4,096-layer native/formatter/dprint-WASM evidence,
  source conservation, following-declaration recovery, and unchanged realistic
  output/topology/allocations.
...
- preserve over-limit body braces and the required language-specific member-list
  shape around one bogus member; preserve a statement as one `BogusStatement`;
- deliberately allow statement overflow to absorb unsupported sibling statements
  up to the enclosing body boundary rather than duplicate `if`/`else`, `do`/
  `while`, and `try`/handler grammar in a second iterative parser;
...
### PR 34 — Unit-test audit

- audit every Rust unit test added since the `a82ab675` stack baseline;
- remove tests whose observable behavior is already covered by formatter or
  syntax corpus fixtures;
- move fixture-representable recovery and boundary behavior into focused corpus
  cases, using generated integration inputs only where a checked-in depth
  adversary would be mechanically large and low-signal;
- retain focused tests only for finite-cost counters, impossible internal
  states, foreign or duplicate source identities, tokenless CST positions, and
  scanner cursor contracts that integration output cannot observe;
- explain beside every retained test why an integration fixture is insufficient,
  and delete test-only helpers and counters that lose their last justified
  client.

The gate is a second baseline-to-head inventory proving every added unit test
has one explicit disposition, plus the full formatter/syntax suite and static
checks. This audit changes test ownership, not formatter or parser behavior.
...
| 34  | `cleanup/34-unit-test-audit`             | draft open | PR 33  | [#36](https://github.com/sargunv/jolt/pull/36) | full + static checks       | Prefer fixtures over implementation-detail tests. |
...
- Kotlin type parsing now abandons an annotation-prefix marker only while that
  marker is empty. A consumed prefix is completed as an `AnnotationList`, so
  malformed `@` types remain inside syntax-owned `BogusType` recovery without
  weakening the parser's strict LIFO marker invariant.
- `FunctionType` and `ContextFunctionType` own their optional leading
  annotations directly. This preserves valid annotated `suspend` and `context`
  function types instead of merely avoiding the panic by misclassifying the soft
  keyword as a user type. All ownership stays in the type grammar/schema.
- One `format_type_annotations` helper replaces the duplicate user-type and
  parenthesized-type loops and formats the two new syntax-owned fields. Parser,
  schema, and formatter production is +53/-52 lines (+1 net), with no new
  collection, cursor, rollback, or recovery framework.
- The exact recovery fixture covers bare `@`, annotated `suspend`, annotated
  `context`, malformed `is @ String`, and a following class. Its syntax and
  formatter snapshots prove diagnostics, token/trivia preservation, following
  syntax recovery, structured valid-type formatting, and non-panic behavior.
- The recovered `when` condition has a present `is` token and a malformed type
  value; current structured formatting already emits `is @ String`. The old
  malformed-keyword spacing proposal remains unreachable, so adding token
  probing or a visibility API was rejected as untestable convenience state.
- Realistic allocation counts/bytes and formatter document topology are exactly
  unchanged. Kotlin syntax child/logical counts increase by 172/688 bytes as
  existing annotated function types gain their declared field; node count and
  reserved bytes are unchanged. Kotlin parse/format medians moved -0.10%/-0.98%;
  Java parse/format moved +1.02%/+0.18%, all neutral. Optimized WASM moved from
  1,746,930 to 1,747,902 bytes (+972, +0.06%).
- `mise run fix` passed workspace formatting, Clippy, dependency, native, and
  WASM checks. The non-update suite passed all 185 tests with zero skips. The
...
- The parser reuses PR 22's calibrated policy value of 128 with a separate
  counter for active syntax owners. Generic lookahead and consumption retain
  their own counter because they must make identical speculative and consuming
  decisions; parser finish asserts both counters return to zero.
- Whole assignment and unary owners cut right-associative, conditional, lambda,
  prefix, cast, parenthesized, call, array-access, and switch-expression cycles.
  Annotation element values share the same active budget across expression/type
  detours. Nested array-initializer children use it without charging the public
  outer initializer.
- At the first unsupported value, one allocation-free borrowed-token scan emits
  a diagnostic-owned `BogusExpression` or a structured `ArrayInitializer`
  containing one `BogusVariableInitializer`. Expression recovery tracks
  conditional `?`/`:` pairs; annotation recovery keeps its distinct comma and
  caller-stop policy. Array recovery preserves its braces and immediate sibling
  values rather than absorbing the parent list.
- Active-owner semantics are explicit: 63 physical nested parentheses remain
  fully structured; the 64th retains all parenthesis nodes around one bogus
  expression. A pure unary or assignment chain remains structured through 126
...
- Production is +182 lines and focused tests are +210 lines. Record-pattern
  ...
  statements. No schema, heap stack, lookahead state, or second expression
  parser was added.
- Tests cover exact depth edges, depth-4,096 unary/assignment/annotation/array
  inputs, annotation ternaries and named siblings, array comma siblings, exact
  diagnostic ownership, lossless source, following fields/classes, and formatter
  completion. Actual dprint-WASM stdin smokes passed independently for all three
  value families and preserved their following declarations.
- Every realistic Java/Kotlin allocation sample, syntax topology, and formatter
  document topology is exactly unchanged. Java parse/format/end-to-end medians
  moved -0.42%/+3.08%/+0.23%, within roughly 5% run MAD. The untouched Kotlin
  path remained noisy; its changes are not attributed to this PR. Optimized WASM
  moved by +3,714 bytes (+0.21%) in the recorded parent/child build.
- `mise run fix` passed strict formatting, workspace Clippy, dependency, native,
...
- The exact `a82ab675`-to-PR-33 inventory contains 54 added or renamed Rust
  source-unit tests. PR 34 removes 35: 12 formatter-IR tests, 15 Java syntax
  tests, and eight Kotlin syntax tests. Nineteen names remain relative to main;
  one is only a renamed pre-existing renderer test, leaving 18 genuinely added
  focused tests.
- Every retained addition now states why integration output is insufficient.
  They cover counted finite-cost bounds, exact recovery-scanner endpoints,
  tokenless CST positions, impossible/foreign source-identity claims, the root
  ignore-plan lifecycle guard, and Kotlin scanner cursor progress. No retained
  unit test merely snapshots a public parse or format result.
- Observable formatter-ignore and delimiter-recovery behavior moved to five
  small Java corpus cases and their syntax/formatter snapshots. Exact/deep Java
  and Kotlin recursion behavior moved to generated formatter integration tests
  that use public typed accessors to prove following declarations remain
  structured before and after formatting, as well as losslessness and
  idempotence.
- Static fixture trials were rejected only with measured evidence: three Kotlin
  expression cases produced a 146,952-byte, 933-line CST snapshot and were not
  compatible with the general formatter corpus diagnostic contract; the Java
  generic and structural trials produced approximately 655 KB and 1.65 MB
  snapshots. Generated integration inputs retain the behavior without checking
  in 2.45 MB of repeated wrapper-node text.
- The PR is +1,035/-1,464 lines overall (-429 net). Rust files under `src/` are
  +56/-1,324 (-1,268 net), entirely in tests or `cfg(test)` scaffolding;
  fixtures, integration tests, and snapshots are +885/-102 (+783 net). The
  obsolete formatter-ignore discovery counter and all displaced tree-counting
  helpers are gone.
- `mise run fix` passes workspace check, strict Clippy, dependency, native, and
  WASM gates. `mise run test` passes all 191 tests with zero skips, including
  the actual dprint-WASM tests and all generated depth adversaries; the parent
  ran 226 tests, exactly accounting for the 35 removed units.
...
| 2026-07-23 | Move observable test behavior out of source-unit tests.          | Corpus fixtures own small parse/format results; generated public integration cases own adversaries whose CST snapshots are mechanically huge. Focused units remain only where output cannot observe the invariant.       |
